...
var camera = [];
for (var i = 0; i <= 3; i++) {
  camera[i] = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.001, 1000);
  camera[i].position.set(0, 0, 150);
  camera[i].lookAt(0, 0, 0);
}
//三维坐标辅助线
var axesHelper=[];
for (var i = 0; i <= 3; i++) {
...
function render() {
  // 设置四个视口的位置和大小
  var viewports = [
    { x: 0, y: halfH, w: halfW, h: halfH }, // 左上
...
async function loadGlobalGUI(){
  ...
  gui.add(parameters.globalvariable, "PCD文件", {
    'three_car三辆车':"../images/min_cut_segmentation_tutorial.pcd",
    'horse驴': "../images/horse.pcd",
    'lioness母狮子': "../images/lioness.pcd",
    'wolf狼': "../images/wolf.pcd",
